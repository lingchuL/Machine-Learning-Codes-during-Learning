...
For the code here, the Logstic_regression.py is a round regression of two-type and the second one is seperate two type of data with a straight line. Visualized like below:
...
Hello World
It's a Three layer(One input, one hidden and one output) full-connect network that learns the handwriting numbers with MNIST's training datas. And the NeuralNetwork_predict.py is using MNIST's test datas to check the accuracy of the traning work. LearnedThetas contains nearly initial trained parameters that means it works with the network but its accuracy is not high.
