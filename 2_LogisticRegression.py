...
#--导入训练数据
data=load(r"F:\SomeMLProjects\ex2data2.txt")
...
class ClassificationLearner(object):
	...
	def optimize(self):
		theta2=np.ones((np.shape(self.X)[1],))
		result=op.minimize(fun=self.getCostFunction,x0=theta2,args=(self.X),method='BFGS',jac=self.gradientDescent)
		print(result)
		return result
	...
	def plotFeature(self,theta,X,y,nowinternum):
		...
		#plt.show()
			...
			#print(self.X[:,1])
			plot_x = np.array([np.min(self.X[:,1]), np.max(self.X[:,1])])
			# print(plot_x)
			plot_y = (-1/self.theta[1])*(self.theta[1]*plot_x+self.theta[0])
			# print(plot_y)
...
if __name__=='__main__':
	Learner=ClassificationLearner(0.1,x,y,theta,100000,1)
	...
	for i in range(1500):
		...
		i_history.append(i)
		if i%500==0 or(i<=100 and i>=50 and i%20==0) or (i<50 and i%10==0):
